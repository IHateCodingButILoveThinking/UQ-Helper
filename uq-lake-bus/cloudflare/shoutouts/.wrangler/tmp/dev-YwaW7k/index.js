...
var COMMENT_TONES = /* @__PURE__ */ new Set(["loved_it", "helpful", "needs_update"]);
...
var MAX_IMAGES_PER_SHOUT = 3;
var MAX_UPLOADS_PER_DAY = 300;
var MAX_UPLOAD_BYTES_PER_DAY = 250 * 1024 * 1024;
var MAX_NETWORK_UPLOADS_PER_DAY = 900;
var MAX_NETWORK_UPLOAD_BYTES_PER_DAY = 750 * 1024 * 1024;
var MAX_NETWORK_WRITES_PER_DAY = 600;
var DEFAULT_STORAGE_LIMIT_BYTES = 8 * 1024 * 1024 * 1024;
  ...
  try {
    ...
    await env.DB.prepare(
      `INSERT INTO food_uploads
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(objectKey, clientHash, detectedType, file.size, width, height, now, now + 60 * 60).run();
  } catch (error) {
...
async function createFoodShout(request, env, respond) {
  ...
  const duplicate = await env.DB.prepare(
    ...
    clientHash,
    title,
    Math.round(latitude * 1e6) - 250,
    Math.round(latitude * 1e6) + 250,
    Math.round(longitude * 1e6) - 250,
    Math.round(longitude * 1e6) + 250,
    now - 24 * 60 * 60
  ).first();
  if (duplicate) {
  ...
  ).bind(...imageKeys, clientHash, now).all();
  ...
  const uploadsByKey = new Map(uploads.map((upload2) => [upload2.object_key, upload2]));
  ...
  const created = await env.DB.prepare(`${foodSelectSql()} WHERE s.id = ? LIMIT 1`).bind(clientHash, clientHash, id).first();
...
__name(updateFoodShout, "updateFoodShout");
...
async function cleanExpiredFoodData(env) {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare("UPDATE food_storage_usage SET used_bytes = MAX(0, used_bytes - ?), updated_at = ? WHERE id = 1").bind(releasedBytes, now),
...
__name(enforceFoodUploadLimit, "enforceFoodUploadLimit");
...
__name(enforceNetworkWriteLimit, "enforceNetworkWriteLimit");
...
__name(logFoodAbuse, "logFoodAbuse");
...
__name(enforceFoodAbuseShield, "enforceFoodAbuseShield");
...
__name(addFoodRisk, "addFoodRisk");
async function reserveFoodStorage(database, byteSize, configuredLimit, now) {
  const requestedLimit = Number(configuredLimit);
  const limit = Number.isFinite(requestedLimit) ? Math.min(10 * 1024 * 1024 * 1024, Math.max(1024 * 1024, Math.trunc(requestedLimit))) : DEFAULT_STORAGE_LIMIT_BYTES;
  await database.prepare(
  ...
  return { usedBytes, maxBytes, percent: Math.min(100, Math.round(usedBytes / maxBytes * 1e3) / 10) };
}
__name(reserveFoodStorage, "reserveFoodStorage");
...
__name(releaseFoodStorage, "releaseFoodStorage");
...
__name(parseFoodImages, "parseFoodImages");
...
__name(normalizeImageKeys, "normalizeImageKeys");
...
__name(getNetworkHash, "getNetworkHash");
...
// .wrangler/tmp/bundle-qBJhhW/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-qBJhhW/middleware-loader.entry.ts
