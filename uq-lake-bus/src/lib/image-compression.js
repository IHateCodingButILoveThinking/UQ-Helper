...
export async function compressFoodImage(file) {
  ...
  if (blob.size > 2_500_000) {
    const smaller = await canvasToBlob(canvas, "image/webp", 0.62);
    if (!smaller || smaller.size > 2_500_000) throw new Error("This photo is still too large after compression.");
